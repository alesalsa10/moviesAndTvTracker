  ...
  try{
    ...
    if(foundUser){
      ...
      if(!profilePicture){
        ...
        let profilePictureLink = profilePicture.split('/');
        let s3Key = profilePictureLink[profilePictureLink.length - 1];
        console.log(s3Key);
        deletePicture(s3Key, function(err){
          if(err){
            res.status(500).json({Msg: 'Something went wrong removing the image'})
          }else {
